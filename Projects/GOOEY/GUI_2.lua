...
--[[
mleb needs to:
change font colour based on syntax highlighting
change background colour based on which user typed it/if it is selected

options:
assign each word typed to a user

assing each character typed to a user
still fairly quick, but uses a tonne of memory

undo/redo:
need a list of changes and where they were made
problem is that the position changes over time, as other

user idea:
a character position is stored per user:
if 2 users are on the same line, one affects the other to appropriately offset it 
...
--[[
	...
	projects need to be public or private
...
CURRENT_USER = {name = "kraftman"} or CURRENT_USER
...
-- these two loops look like they can be merged to one

	for i = #UIParent.__LAYERS, 1, -1 do
		for j = #UIParent.__LAYERS[i],1,-1 do
			if UIParent.__LAYERS[i][j]:MouseUp(x,y,button,j) then
				return
			end
		end
	end
...
function UIParent:keypressed(key,uni)
	...
	if key == "lshift" or key == "rshift" then
		self.__VARS.shiftdown = true
	elseif key == "lctrl" or key == "rctrl" then
		self.__VARS.ctrldown = true
	elseif key == "ralt" or key == "lalt" then
		self.__VARS.altdown = true
	end
...
function UIParent:keyreleased(key,uni)
	...
	if key == "lshift" or key == "rshift" then
		self.__VARS.shiftdown = false
	elseif key == "lctrl" or key == "rctrl" then
		self.__VARS.ctrldown = false
	elseif key == "ralt" or key == "lalt" then
		self.__VARS.altdown = false
	end
...
function IsShiftKeyDown()
	return self.__VARS.shiftdown
end

function IsCtrlKeyDown()
	return self.__VARS.ctrldown
end

function IsAltKeyDown()
	return self.__VARS.altdown
end
...
function Frame:MouseUp(x,y,button,index)
	for i = 1, #self.__LAYERS do
		for j =  #self.__LAYERS[i],1,-1 do
			if self.__LAYERS[i][j].MouseUp then
				if self.__LAYERS[i][j]:MouseUp(x,y,button,j) then
					return true
				end
			end
		end
...
function MultiLineEditBox:Create(parent, ...)
	...
	temp.__VARS.font = love.graphics.newFont(temp.__VARS.fontsize)
...
function MultiLineEditBox:MouseDown(x,y,button,index,user)
	...
	if x > self:GetLeft() and x < self:GetRight() and y > self:GetTop() and y < self:GetBottom() then
			user.selectededitbox = nil
			for _,eb in pairs(UIParent.EditBoxs) do
				if eb == self then
					eb.__VARS.focus = true
					self:MoveCursor(x,y,user) -- set the cursor to the correction location
					if not self.users[user] then
						self.users[user] = user
					end
					local s = self.users[user]
					s.downline = floor((y - self:GetTop()+(self:GetTop()-self.__VARS.lines[1]:GetTop()))/self.__VARS.fontsize)+1
					s.downpos = math.min(floor(((x-self:GetLeft())/(self.__VARS.font:getWidth(self.__VARS.lines[user.selectedline].text)/self.__VARS.lines[user.selectedline].text:len()))),self.__VARS.lines[user.selectedline].text:len()) 
					self:SetScript("OnUpdate", function(self) 
							local x, y = love.mouse.getPosition()
							s.upline = floor((y - self:GetTop()+(self:GetTop()-self.__VARS.lines[1]:GetTop()))/self.__VARS.fontsize)+1
							s.uppos = math.min(floor(((x-self:GetLeft())/(self.__VARS.font:getWidth(self.__VARS.lines[user.selectedline].text)/self.__VARS.lines[user.selectedline].text:len()))),self.__VARS.lines[user.selectedline].text:len()) 

					end)
					print(self.users[user].downline, self.users[user].downpos)
				else
					eb.__VARS.focus = nil
				end
			end
...
function MultiLineEditBox:MouseUp(x,y,button,index,user)
	user = user or CURRENT_USER
	for i = 1, #self.__LAYERS do
		for j =  #self.__LAYERS[i],1,-1 do
			if self.__LAYERS[i][j].MouseUp then
	...
	self.users[user].upline = floor((y - self:GetTop()+(self:GetTop()-self.__VARS.lines[1]:GetTop()))/self.__VARS.fontsize)+1
	self.users[user].uppos = math.min(floor(((x-self:GetLeft())/(self.__VARS.font:getWidth(self.__VARS.lines[user.selectedline].text)/self.__VARS.lines[user.selectedline].text:len()))),self.__VARS.lines[user.selectedline].text:len()) 
	self:SetScript("OnUpdate", nil)
				print(self.users[user].upline, self.users[user].uppos)
...
function MultiLineEditBox:MoveCursor(x,y,user)
	user = user or CURRENT_USER
		
		user.selectededitbox = self
		user.selectedline = floor((y - self:GetTop()+(self:GetTop()-self.__VARS.lines[1]:GetTop()))/self.__VARS.fontsize)+1
		if not self.__VARS.lines[user.selectedline] then
			user.selectedline = #self.__VARS.lines
		end
		
		self:SelectLine(user.selectedline, user)
		
		if self.__VARS.selectedline.text:len() > 0  then
			local cpos = floor(((x-self:GetLeft())/(self.__VARS.font:getWidth(self.__VARS.lines[user.selectedline].text)/self.__VARS.lines[user.selectedline].text:len()))) --set the character position
			user.linepos = math.min(cpos, self.__VARS.lines[user.selectedline].text:len())
		else
			user.linepos = 0
		end
end

function MultiLineEditBox:SelectLine(index, user) --tells a line who is currently using it, so it can write to FB or not
...
function MultiLineEditBox:keypressed(key,uni, user)
	
	user = user or CURRENT_USER

	local sl = self.__VARS.lines[user.selectedline]
	
	if key == "escape" then
		self:ClearFocus()
	elseif key == "left" then
		if user.linepos == 0 then
			if sl.index > 1 then
				self:SelectLine(sl.index-1,user)
				user.linepos = self.__VARS.selectedline.text:len()
				user.selectedline = user.selectedline - 1
			end
		else
			user.linepos = user.linepos -1
		end
	elseif  key == "right" then
		if user.linepos == sl.text:len() then
	...
	elseif key == "up" then
		if sl.index > 1 then
			self:SelectLine(sl.index-1,user)
			user.linepos = min(self.__VARS.selectedline.text:len(), user.linepos)
		end
	elseif key == "down" then
		if sl.index < #self.__VARS.lines then
			self:SelectLine(sl.index+1,user)
			user.linepos = math.min(self.__VARS.selectedline.text:len(), user.linepos)
		end
	elseif key == "backspace" then
		if user.linepos == 0 then
			if sl.index > 1 then
			...
			sl:keypressed(key,uni,user)
		end
	elseif key == "return" then
		local temp = sl.text
		sl.text = sl.text:sub(1, user.linepos)
		self:AddLine(sl.index+1, temp:sub(user.linepos+1))
		self:SelectLine(sl.index+1, user)
		user.selectedline = user.selectedline + 1
		user.linepos = 0 --not sure this needs to be here?
	else
		sl:keypressed(key,uni,user)
	end
...
function MultiLineEditBox:Draw()
	...
	for i = 1, #self.__VARS.lines do
		...
		love.graphics.setColor(255,255,255,70)
			...
			for user, data in pairs(self.users) do
				y = l:GetTop()
				height = l:GetHeight()

				if l.index == data.downline then
					if data.downline == data.upline then
						x = self.__VARS.font:getWidth(l.text:sub(1, data.downpos))
						width = self.__VARS.font:getWidth(l.text) - x  - self.__VARS.font:getWidth(l.text:sub(data.uppos))
					else
						x = self.__VARS.font:getWidth(l.text:sub(1, data.downpos))
						width = self.__VARS.font:getWidth(l.text) - x
					end
					love.graphics.rectangle("fill", x+self:GetLeft(), y, width, height)
				elseif (l.index > data.downline) and (l.index < data.upline) then
					love.graphics.rectangle("fill", self:GetLeft(), y, self.__VARS.font:getWidth(l.text), height)
				elseif l.index == data.upline then
					x = self:GetLeft()
					width = self.__VARS.font:getWidth(l.text:sub(1,data.uppos))
		...
		if l.active then
			...
			l:Draw()
			for u in pairs(l.users) do
				local x
				if self.__VARS.selectedline.text:len() > 0 then
				 x = l:GetLeft()+self.__VARS.font:getWidth(l.text:sub(1, u.linepos)) --needs updating to draw the line in the right place
				 else
				 x = l:GetLeft()
				end
				love.graphics.line(x, l:GetTop(),x, l:GetBottom())
			end
		elseif l.buffer then
...
function MLEBLine:keypressed(key, uni, user)

	user = user or CURRENT_USER

	if key == 'backspace' then
			self.text = self.text:sub(1, user.linepos-1)..self.text:sub(user.linepos+1)
			
			for u in pairs(self.users) do -- for all other users on this line
					if u.linepos<= user.linepos then
						u.linepos = u.linepos -1 --if its after the removed character, it needs to shift to the left
					end
			end
	elseif key == "return" then
		if self.__SCRIPTS.OnEnterPressed then
	...
	elseif key == "tab" then
		self.text = self.text:sub(1, user.linepos).."    "..self.text:sub(user.linepos+1)
		
		for u in pairs(self.users) do
			if u.linepos<= user.linepos then
				u.linepos = u.linepos +4 --if its after the removed character, it needs to shift to the left
			end
		end
	elseif uni > 31 and uni < 127 then
		self.text = self.text:sub(1, user.linepos)..string.char(uni)..self.text:sub(user.linepos+1)
		for u in pairs(self.users) do
			if u.linepos<= user.linepos then
				u.linepos = u.linepos+1 --if its after the removed character, it needs to shift to the left
			end
